...
async function main() {
  ...
  if (!existingTecnico) {
    ...
    console.log('✅ Técnico criado: tecnico@prefeitura.rio')
  } else {
    console.log('⏭️  Técnico já existe, pulando...')
  }

  // Verifica se já tem crianças
  const existingChildren = await prisma.child.count()
  if (existingChildren > 0) {
    console.log(`⏭️  ${existingChildren} crianças já cadastradas, pulando seed de crianças...`)
    return
  }
  ...
  if (!fs.existsSync(seedPath)) {
    console.error(`❌ seed.json não encontrado em: ${seedPath}`)
    process.exit(1)
  }
  ...
  console.log(`✅ ${children.length} crianças importadas com sucesso!`)
