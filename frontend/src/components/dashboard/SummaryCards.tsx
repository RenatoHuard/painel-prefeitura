...
export function SummaryCards({ summary }: Props) {
  ...
  const cards = [
    ...
    { label: 'Sem Dados em Nenhuma Área', value: summary.semDados, icon: DatabaseZap, color: 'text-rose-500', bg: 'bg-rose-500/10', border: 'border-rose-500/20', sub: 'Requer cadastramento urgente', href: '/children?semDados=true' },
  ]
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {cards.map((card) => (
          ...
          >
            <Card className={`border ${card.border} hover:shadow-md hover:scale-[1.02] transition-all duration-150 h-full`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
        {areCards.map((card) => (
          <button
            ...
            aria-label={`Ver ${card.label}`}
          >
            <Card className="hover:shadow-md hover:scale-[1.02] transition-all duration-150 h-full">
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <span className="text-xs text-muted-foreground/40 group-hover:text-muted-foreground transition-colors" aria-hidden="true">→</span>
              </CardContent>
            </Card>
          </button>
        ))}
